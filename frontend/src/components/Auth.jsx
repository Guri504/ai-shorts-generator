...
export default function Auth() {
  ...
  const { signup, login, authError, isAuthLoading } = useAppStore();

  useEffect(() => {
    // Detect token in query parameters (redirected from Google Login)
    if (typeof window !== 'undefined') {
      const params = new URLSearchParams(window.location.search);
      const token = params.get('token');
      if (token) {
        // Hydrate token immediately
        localStorage.setItem('token', token);
        useAppStore.setState({ token, isAuthenticated: true, isAuthLoading: true });
        
        // Fetch profile to complete authentication
        const fetchUserProfile = async () => {
          try {
            ...
            useAppStore.setState({ user, isAuthLoading: false, activeTab: 'dashboard' });
          } catch (err) {
            console.error('[Google OAuth Hydration] Failed to fetch user profile:', err);
            localStorage.removeItem('token');
            useAppStore.setState({ token: null, isAuthenticated: false, isAuthLoading: false, authError: 'Failed to verify Google Login. Please try again.' });
          }
        };
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          >
            {isAuthLoading ? (
              ...
              <>
                <Sparkles size={14} />
                {isLogin ? 'Login to Dashboard' : 'Create SaaS Account'}
              </>
            )}
          </button>
